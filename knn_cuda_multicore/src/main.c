...
#include "knn.h"
	...
	unsigned char key[NUM_TESTING_SAMPLES] = {
		#if K == 20
			#include "data/key-READ1-k20.dat"
		#elif K == 3
			#include "data/key-READ1-k3.dat"
		#endif
	};
#else
...
int main() {
    ...
    printf("kNN: number of classified instances = %d\n", num_new_points);
		...
		printf("\nTime:  %.4f s\n", time);
		const char *existing_file_name = "C:/Dev/EHCLab/knn_cuda/build/profile.txt";
		FILE *existing_file = fopen(existing_file_name, "a");
		if (existing_file == NULL) {
			perror("Error opening existing file for appending");
			return EXIT_FAILURE;
		}
		fprintf(existing_file, "%.6f\n", time);
